...
void analyze_emcal_ht(const char *fname = "cemc_towers_02_21598.root", const char *outfname = "hot_tower_analysis_02_21598.root") {	

	int nch = 0;
    std::vector<int> *t_channel = nullptr;
    TFile *in = new TFile(fname);
    TTree *httree = (TTree*)in->Get("T");
    httree->SetBranchAddress("channel", &t_channel);
    httree->GetEntry(0);
    nch = (int)(*t_channel).size();
    in->Close();
    ...
    for (int i = 0; i < nch; i++) {
        h_chi2_towers[i] = new TH1F(TString::Format("chi2_tower_%d",(*t_channel)[i]),"",100000,0,100000);
        h_e_towers[i] = new TH1F(TString::Format("e_tower_%d",(*t_channel)[i]),"",64000,-640000,640000);
        h_pedestal_towers[i] = new TH1F(TString::Format("pedestal_tower_%d",(*t_channel)[i]),"",16384,0,16384);
    }
    ...
    for (int i = 0; i < nch; i++) {
        ...
        chi2_avg += mean;
    	if (mean > 1 || pmean > 3000) {
    		std::cout << "hot tower at channel " << (*channel)[i] << " with average chi2 " << mean << " average energy " << emean << " and average pedestal " << pmean << std::endl;
    		ht_channel.push_back((*channel)[i]);
