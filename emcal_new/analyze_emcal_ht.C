...
void analyze_emcal_ht(const char *fname = "cemc_towers_02_21598.root", const char *outfname = "hot_tower_analysis_02_21598.root") {	
	...
	for (int i = 0; i < 8; i++) {
		for (int j = 0; j < 384; j++) {
            h_channels[384*i+j] = new TH1F(TString::Format("channel_%d",sectors[i]*384+j),"",3000,0,3000);
			h_chi2_towers[384*i+j] = new TH1F(TString::Format("chi2_tower_%d",sectors[i]*384+j),"",100000,0,100000);
